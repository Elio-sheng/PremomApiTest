...
class UserService(RestClient):
    ...
    def userDelete(self, **kwargs):  # 删除账号
        return self.request("/user/user/cancellation/account/delete/reason",
                            method="POST",
                            **kwargs)
    ...
    def userGuarantee(self, **kwargs):  # 加入包孕计划接口
        return self.request("/user/guarantee/subscribe",
                            method="POST",
                            **kwargs)
    ...
    def userMemberV2PageInfo(self,
                             **kwargs):  # 会员onboarding或landing page2个页面的数据
        return self.request("/user/member/v2/page/info",
                            method="POST",
                            **kwargs)
