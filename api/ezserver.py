...
class EzserverService(RestClient):
    ...
    def addSignsRecords_json(self, **kwargs):   #算法
        return self.request("/easerver/sync/addSignsRecords.json", method="POST", **kwargs)

    def replaceAlgorithmData(self, **kwargs):   #算法
        return self.request("/ezserver/api/cycle/replaceAlgorithmData", method="POST", **kwargs)

    def window_policy(self, **kwargs):   #算法
        return self.request("/ezserver/api/window/policy", method="POST", **kwargs)
