...
class Report():
    # 获取用户周期分析报告列表
    def AnalysisList(self, title, except_result, expect_code, expect_msg,
                     core_token):
        header = {
            "apiVersion": '42',
            "appVersion": '1.35.0',
    ...
    # 获取用户周期状态
    def CycleStatus(self, title, cycleNumber, except_result, expect_code,
    ...
    # 分享用户周期分析报告
    def ShareAnalysis(self, title, cycleNumber, email, cycleEndNumber, id,
                      except_result, expect_code, expect_msg, core_token):
        header = {
    ...
    # 获取周期比较报告列表
    def ComparisonList(self, title, except_result, expect_code, expect_msg,
                       core_token):
        header = {
            "apiVersion": '43',
            "appVersion": '1.36.0',
    ...
    # 分享用户周期比较报告
    def ShareComparison(self, title, cycleNumber, email, cycleEndNumber, id,
                        except_result, expect_code, expect_msg, core_token):
        header = {
            "Content-Type": "application/json",
            "apiVersion": '43',
    ...
    def CervicalAndMoodAndSymptom(self, title, expect_code, expect_msg,
                                  get_userId):
        header = {
            "apiVersion": '43',
            "appVersion": '1.36.0',
        ...
        json_data = {"userId": get_userId, "cycleNumber": '1'}
    ...
    def AnalysisPeakFertility(self, title, expect_code, expect_msg,
                              get_userId):
        header = {
            "apiVersion": '43',
            "appVersion": '1.36.0',
    ...
    def GlanceIntercourseTiming(self, title, expect_code, expect_msg,
                                core_token):
        header = {
            "apiVersion": '43',
            "appVersion": '1.36.0',
...
class Insemination():
    ...
    def inseminationAddOrUpdate(self, title, id, frozenSperm, noInsemination,
                                noneOfTheAbove, triggerShot, type, expect_code,
                                expect_msg, core_token, get_current_date):
        """增加或更新受精记录"""
        header = {
    ...
    def inseminationDataGet(self, title, get_current_date, expect_code,
                            expect_msg, core_token):
        """根据日期，获取受经记录"""
        header = {
        ...
        data = {"recordDate": get_current_date}
        ...
        assert res.json()['id'] in str(
            sql_res[0]["id"])  # 断言当前生成的记录id：返回的id=数据库存的id
    ...
    def dailyRecordGetSetting(self, title, expect_code, expect_msg,
                              core_token):
        """用户每日记录设置"""
        header = {
            "Content-Type": "application/x-www-form-urlencoded",
    ...
    def inseminationGetTip(self, title, editCycle, editDate, expect_code,
                           expect_msg, core_token):
        """获取提示文案"""
        header = {
            "Content-Type": "application/x-www-form-urlencoded",
        ...
        data = {"editCycle": editCycle, "editDate": editDate}
    ...
    def inseminationLogSexKeepOrChangeBtn(self, title, keepLogSex, expect_code,
                                          expect_msg, core_token):
        """保留或更换home页面LogSex按钮"""
        header = {"Content-Type": "application/json", "authToken": core_token}
        json_data = {"keepLogSex": keepLogSex, "authToken": core_token}
        res = core.inseminationLogSexKeepOrChangeBtn(headers=header,
                                                     params=json_data)
        ResultBase(res, expect_code, expect_msg, expect_msg,
                   res)  # 断言code和message

    def dailyRecordUpdateSetting(self, title, enable, order, settingType,
    ...
    def dailyRecordMomentGet(self, title, expect_code, expect_msg, core_token,
                             get_current_date):
        """获取用户Moment分享-每日记录数据"""
        header = {
            "Content-Type": "application/json;charset=UTF-8",
        ...
        json_data = {"queryDay": get_current_date}
    ...
    def prePregnancySave(self, title, height, heightUnit, weight, weightUnit,
                         expect_code, expect_msg, core_token):
        """保存孕前信息记录"""
        header = {"Content-Type": "application/json", "authToken": core_token}
        json_data = {
    ...
    def mooeGet(self, title, recordDate, expect_code, expect_msg, core_token):
        ...
        json_data = {"recordDate": recordDate}
    ...
    def medicineGalleryUpdate(self, title, category, categoryOrder, deleted,
                              intakeNumber, medicineId, name, order, reminder,
                              reminderTime, type, expect_code, expect_msg,
                              core_token):
        """更新用户药品记录"""
        header = {"Content-Type": "application/json", "authToken": core_token}
        json_data = {
    ...
    def medicineGet(self, title, recordDate, expect_code, expect_msg,
                    core_token):
        """获取用户服药记录"""
        header = {
            "Content-Type": "application/x-www-form-urlencoded",
    ...
    def medicineUpdate(self, title, category, categoryName, categoryOrder,
                       enable, medicineId, name, order, recordDate,
                       expect_code, expect_msg, core_token):
        """更新用户服药记录"""
        header = {"Content-Type": "application/json", "authToken": core_token}
        json_data = {
    ...
    def spermLogAdd(self, title, images, progressiveMotility, spermCount, time,
                    totalMotility, expect_code, expect_msg, core_token):
        """增加精子记录"""
        header = {"Content-Type": "application/json", "authToken": core_token}
        json_data = {
            "images": [images],
    ...
    def spermDataGet(self, title, recordDate, expect_code, expect_msg,
                     core_token):
        """根据日期，获取精子记录"""
        header = {
            "Content-Type": "application/x-www-form-urlencoded",
    ...
    def spermLogHistoryGet(self, title, page, size, expect_code, expect_msg,
                           core_token):
        """获取用户精子历史数据列表"""
        header = {
            "Content-Type": "application/x-www-form-urlencoded",
