...
class Ezserver:
    ...
    def InputAl(self, title, BBT, menstruationRecord, expect_code, expect_msg,
                ez_token):
        header = {"Content-Type": "application/json", "authToken": ez_token}
        json_data = {
            "daysInput": [{
    ...
    def addSignsRecords_json(self, title, notes, expect_code, expect_msg,
                             ez_token):
        header = {"Content-Type": "application/json", "authToken": ez_token}
        json_data = {
            "records": [{
        ...
        res = EzserverService().addSignsRecords_json(headers=header,
                                                     json=json_data)
        logger.info(res)
        logger.info("预期msg===>> {}".format(expect_msg))
        logger.info("实际msg===>> {}".format(res.text))
